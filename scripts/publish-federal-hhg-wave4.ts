/**
 * Publish Task 010 Wave 4 final clean federal HHG carriers from Task 010 manifest.
 * Carriers only. Never inserts identity-review, inactive, brokers, or duals.
 * Never calls Google Places. Rollback isolates Wave 4 from Waves 1–3.
 *
 * npm run publish:federal-hhg-wave4 -- --dry-run --limit 50
 * npm run publish:federal-hhg-wave4 -- --limit 50
 * npm run publish:federal-hhg-wave4 -- --limit 1000
 * npm run publish:federal-hhg-wave4 -- --indexable
 * npm run publish:federal-hhg-wave4 -- --rollback
 */
...
const MANIFEST_PATH = resolve(process.cwd(), 'docs/task-010-final-clean-selection.json');
...
function loadManifest(): ManifestFile {
  ...
  if (!Array.isArray(parsed.candidates)) throw new Error('Wave 4 manifest missing candidates');
  return parsed;
}
...
async function main() {
  ...
  const limit = Math.max(0, argNum('--limit', 50));
  ...
  `);
  const priorWaves = await client.query(
      ...
      JSON.stringify(
        {
          rollback: true,
          dry,
          wave4: WAVE_4_PUBLICATION_ID,
          prior_waves: priorWaves.rows,
    ...
    if (!dry) {
      await client.query(
        `UPDATE public.companies c
          ...
          WHERE w.wave_id = $1
            ...
            AND w.status IN ('published', 'indexable')
      ...
      await client.query(
        `UPDATE public.federal_hhg_wave_publication
          ...
          WHERE wave_id = $1 AND status IN ('published', 'indexable')`,
        [WAVE_4_PUBLICATION_ID]
      );
    }
  ...
  const manifestDots = loadWave4ManifestUsdots(manifest.candidates);
  ...
  for (const candidate of manifest.candidates) {
    ...
    const check = revalidateWave4Candidate(stagedRow, manifestDots, existingDots);
    if (!check.ok) {
  ...
  const selectedMeta =
    alreadyUsdots.size === 0 && limit <= 100
      ? selectWave4Canary(eligibleMeta, remaining)
  ...
  writeFileSync(
    resolve(docsDir, 'task-010-wave4-publish-selection.json'),
    JSON.stringify(
      {
        google_places_requests: 0,
  ...
  for (const row of selected) {
    ...
    prepared.push({
      ...
      row,
      hq,
      short: `Federally authorized interstate household-goods motor carrier (USDOT ${usdot}). Confirm the current FMCSA SAFER record before booking.`,
      description: `${name} is listed from FMCSA Licensing & Insurance evidence as an interstate household-goods motor carrier. Legal name: ${row.legal_name}. Headquarters on the federal record: ${hq}. ${row.mc ? `MC-${row.mc}. ` : ''}This profile does not claim nationwide local coverage, auto-transport authority, or consumer ratings that are not yet on file.`,
  ...
  const priorAfter = await client.query(
    `SELECT wave_id, count(*)::int AS n
       FROM public.federal_hhg_wave_publication
      WHERE status <> 'unpublished'
      GROUP BY 1 ORDER BY 1`
  ...
  const report = {
    ...
    wave: wave.rows,
    selected: waveSelectionStats(selected),
    prior_waves: priorAfter.rows,
    protected: protectedNow.rows,
  };
  writeFileSync(resolve(docsDir, 'task-010-wave4-publish.json'), JSON.stringify(report, null, 2) + '\n');
