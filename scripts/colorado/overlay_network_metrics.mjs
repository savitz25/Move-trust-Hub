/**
 * Overlay CO-MOVE-001 onto committed move-network-metrics-v1 without a Supabase recount.
 */
...
if (prev.network.publishedStateIntelligencePaths.includes('/colorado')) {
  throw new Error('CO already present in network metrics; reset from main before overlay');
}
...
const input = {
  ...
  coActiveHhgPermitListings: snap.active_universe.official_total_permits,
  coRevokedHhgListings: snap.status_classes.classes.REVOKED.official_total,
  coSuspendedHhgListings: snap.status_classes.classes.SUSPENDED.official_total,
  coSourceAsOf: snap.source.source_publication_date,
  publishedStateIntelligencePaths: [...prev.network.publishedStateIntelligencePaths, '/colorado'],
...
console.log('overlay', {
  ...
  coActive: next.colorado.activeHhgPermitListings,
});
