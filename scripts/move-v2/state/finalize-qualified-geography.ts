...
async function main() {
  ...
  const wave=process.env.MOVE_STATE_WAVE||"STATE_004B";
  await c.query(`delete from move_v2.provider_geography_evidence g using move_v2.provider_local_eligibility e
    where g.provider_id=e.provider_id and e.superseded_at is null and e.eligibility='STATE_VERIFIED_MOVING_BROKER'
      and exists(select 1 from move_v2.enrichment_queue q where q.provider_id=e.provider_id and q.wave=$1)`,[wave]);
  const result = await c.query(`insert into move_v2.provider_geography_evidence
    ...
    where e.superseded_at is null and e.eligibility='STATE_VERIFIED_LOCAL_MOVER'
      and exists(select 1 from move_v2.enrichment_queue q where q.provider_id=e.provider_id and q.wave=$2)
    ...
    returning provider_id,evidence_status,derived_service_area_required` , [GEOGRAPHY_RULE_VERSION,wave]);
