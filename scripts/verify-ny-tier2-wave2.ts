/**
 * Locked Tier 2 contract checks for NY Wave 2.
 * Run: npx tsx scripts/verify-ny-tier2-wave2.ts
 */
import {
...
const FORBIDDEN =
  /\b(FDACS|Chapter 507|BHGS|TxDMV|NJ BPU|Board of Public Utilities|PA PUC|UTC household goods permit|NCUC)\b/i;

const failures: string[] = [];

for (const slug of NY_TIER2_WAVE2) {
  if ((NY_TIER1_CORE12 as readonly string[]).includes(slug)) {
    failures.push(`${slug}: listed in NY_TIER1_CORE12 — skip Tier 2 rebuild`);
  }
  if ((NY_TIER2_WAVE1 as readonly string[]).includes(slug)) {
    failures.push(`${slug}: listed in NY_TIER2_WAVE1 — do not rebuild Wave 1`);
  }

  const pack = getCountyIntelligencePack('new-york', slug);
  if (!pack) {
  ...
  if (!/NYSDOT|household-goods|household goods/i.test(blob)) {
    failures.push(`${slug}: missing NYSDOT / household-goods language`);
  }
  if (!/FMCSA/i.test(blob)) failures.push(`${slug}: missing FMCSA`);
  if (FORBIDDEN.test(blob)) failures.push(`${slug}: foreign regulator bleed`);
  if (pack.stateSlug !== 'new-york') failures.push(`${slug}: wrong stateSlug`);
...
if (failures.length) {
  console.error('\nNY TIER 2 WAVE 2 CONTRACT FAILURES:');
  for (const f of failures) console.error(' -', f);
  process.exit(1);
}

console.log('\nNY Tier 2 Wave 2 contract checks passed.');
