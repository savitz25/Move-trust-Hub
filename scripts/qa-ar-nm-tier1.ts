/**
 * QA for Arkansas Core 6 + New Mexico Core 5 + TX Travis / AZ Maricopa regression.
 * Run: npx tsx scripts/qa-ar-nm-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'arkansas') {
    checks.ardotLanguage =
      /ardot|arkansas intrastate|intrastate operating authority|department of transportation \(ardot\)/i.test(
        heroBlob + faqText + packBlob
      );
    checks.noForeignRegulators = !/occ household|txdmv|modot|lpsc|nj public-mover|tdor|nmdot trb/i.test(
      `${pack.heroCredibility} ${pack.heroOpener}`
    );
    if (slug === 'washington') {
      checks.washingtonArDisambig =
        /arkansas|fayetteville|nwa|northwest arkansas/i.test(heroBlob + h1) &&
        !/\bmoving in washington state\b|\bwashington county,?\s*utah\b movers/i.test(
          heroBlob + h1
        );
    }
    if (slug === 'benton') {
      checks.bentonArDisambig = /bentonville|rogers|nwa|northwest arkansas/i.test(
        heroBlob + h1
      );
    }
  }
  if (stateSlug === 'new-mexico') {
    checks.nmTrbLanguage =
      /nmdot|trb|transportation regulation bureau|public regulation commission|household goods/i.test(
        heroBlob + faqText + packBlob
      );
    checks.noForeignRegulators = !/txdmv|colorado puc|udot|nj public-mover|ardot|fdacs/i.test(
      `${pack.heroCredibility} ${pack.heroOpener}`
    );
    if (slug === 'bernalillo') {
      checks.bernalilloDisambig = /albuquerque|heights|valley|i-25|i-40/i.test(heroBlob + h1);
    }
    if (slug === 'sandoval') {
      checks.sandovalDisambig = /rio rancho|sandoval/i.test(heroBlob + h1);
    }
    if (slug === 'doa-ana') {
      checks.doaAnaDisambig = /las cruces|doña ana|dona ana/i.test(heroBlob + h1);
    }
  }
...
const rows: Row[] = [
  ...AR_TIER1_CORE6.map((slug) => auditCounty('arkansas', slug)),
  ...NM_TIER1_CORE5.map((slug) => auditCounty('new-mexico', slug)),
];
rows.push(auditCounty('texas', 'travis'));
rows.push(auditCounty('arizona', 'maricopa'));

const arH1s = rows.filter((r) => r.county.startsWith('arkansas/')).map((r) => r.h1);
const nmH1s = rows.filter((r) => r.county.startsWith('new-mexico/')).map((r) => r.h1);
const h1DistinctAr = new Set(arH1s).size === arH1s.length;
const h1DistinctNm = new Set(nmH1s).size === nmH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  arCore6: AR_TIER1_CORE6.length,
  nmCore5: NM_TIER1_CORE5.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossAr: h1DistinctAr,
  h1DistinctAcrossNm: h1DistinctNm,
  scaleRecommendation:
    rows.every((r) => r.pass) && h1DistinctAr && h1DistinctNm
      ? 'Arkansas Core 6 + New Mexico Core 5 Tier-1 complete on locked template.'
      : 'Fix failing markets before calling AR/NM Core complete',
  arSummary: rows
    .filter((r) => r.county.startsWith('arkansas/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  nmSummary: rows
    .filter((r) => r.county.startsWith('new-mexico/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
      national: r.nationalCount,
  ...
  weakLocalCounties: rows
    .filter(
      (r) =>
        (r.county.startsWith('arkansas/') || r.county.startsWith('new-mexico/')) &&
        r.localCount === 0
    )
    .map((r) => r.county),
  rows,
...
writeFileSync('scripts/output/ar-nm-tier1-qa.json', JSON.stringify(report, null, 2));
console.log(JSON.stringify(report, null, 2));
process.exit(report.failCount > 0 || !h1DistinctAr || !h1DistinctNm ? 1 : 0);
