...
for (const [slug, expect] of wave1) {
  try {
    ...
    const okReg = /TDOR|Department of Revenue|motor carrier/i.test(html);
    const pass = status === 200 && okH1 && okParent && okDate && okReg;
    if (!pass) fail++;
    console.log(
      `${pass ? 'PASS' : 'FAIL'} | ${slug.padEnd(14)} | ${String(status).padEnd(3)} | ${okParent ? 'yes' : 'NO '} | ${okDate ? 'date-ok' : 'BAD-DATE'} | ${okReg ? 'tn-reg' : 'NO-REG'} | ${h1 || '(missing)'}`
    );
  } catch (e) {
    fail++;
