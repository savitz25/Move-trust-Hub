/**
 * QA for Indiana Tier-1 Core 8 + IL Cook / OH Franklin regression.
 * Run: npx tsx scripts/qa-in-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'indiana') {
    checks.inDorLanguage =
      /department of revenue|dor|ic 8-2\.1-22|certificate of public convenience|indiana operating authority/i.test(
        heroBlob + faqText + packBlob
      );
    checks.noForeignRegulators = !/puco|icc household|msp cved|utc household|odot|fdacs|txdmv|nysdot|bhgs|tdor|pa puc|va dmv|nj public-mover|mndot|dpu operating/i.test(
...
const rows: Row[] = IN_TIER1_CORE8.map((slug) => auditCounty('indiana', slug));
rows.push(auditCounty('illinois', 'cook'));
rows.push(auditCounty('ohio', 'franklin'));

const inH1s = rows.filter((r) => r.county.startsWith('indiana/')).map((r) => r.h1);
const h1Distinct = new Set(inH1s).size === inH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  inCore8: IN_TIER1_CORE8.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossIn: h1Distinct,
  scaleRecommendation: rows.every((r) => r.pass) && h1Distinct
    ? 'Indiana Core 8 Tier-1 complete on locked template. Next: onboard true IN local movers for local-first segments where weak.'
    : 'Fix failing counties before calling IN Core 8 complete',
  summaryTable: rows
    .filter((r) => r.county.startsWith('indiana/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  weakLocalCounties: rows
    .filter((r) => r.county.startsWith('indiana/') && r.localCount === 0)
    .map((r) => r.county),
  rows,
};

mkdirSync('scripts/output', { recursive: true });
writeFileSync('scripts/output/in-tier1-qa.json', JSON.stringify(report, null, 2));
