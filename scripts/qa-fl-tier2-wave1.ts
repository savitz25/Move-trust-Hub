/**
 * QA for Florida Tier 2 Wave 1 content contract.
 */
...
const PARENT_HINTS: Record<string, RegExp> = {
  polk: /Hillsborough|Tampa|Orange|Orlando/i,
  brevard: /Orange|Orlando/i,
  volusia: /Orange|Orlando|Duval/i,
  pasco: /Hillsborough|Tampa/i,
  manatee: /Hillsborough|Tampa|Sarasota/i,
  collier: /Lee|Fort Myers|Naples/i,
  seminole: /Orange|Orlando/i,
  osceola: /Orange|Orlando/i,
  lake: /Orange|Orlando/i,
  'st-lucie': /Palm Beach|PBC|Treasure/i,
  marion: /Orange|Independent|Central Florida|Ocala/i,
  escambia: /Duval|Jacksonville|Panhandle|Independent|Pensacola/i,
};
...
function audit(slug: string): Row {
  ...
  checks.parentHint = PARENT_HINTS[slug]
    ...
    : true;
  checks.notCore9 = !(FL_TIER1_CORE9 as readonly string[]).includes(slug);
...
const rows = FL_TIER2_WAVE1.map(audit);
const passed = rows.filter((r) => r.pass).length;

const core9ok = (FL_TIER1_CORE9 as readonly string[]).every((slug) => {
  if (slug === 'sarasota') {
    // Sarasota is protected as Tier 1 core conceptually; pack may not exist yet.
...
const txFortBend = getCountyIntelligencePack('texas', 'fort-bend');
const txOk = Boolean(
  txFortBend?.contentTier === 'tier2' &&
    txFortBend.parentCompare?.bullets?.length &&
    txFortBend.h1
);

const report = {
  wave1: FL_TIER2_WAVE1.length,
  passed,
  core9Intact: core9ok,
  txFortBendTier2Regression: txOk,
  rows,
};

writeFileSync(
  'scripts/qa-fl-tier2-wave1-report.json',
  JSON.stringify(report, null, 2)
);
console.log(JSON.stringify(report, null, 2));
console.log(
  `\nFL Tier2 Wave1 ${passed}/${FL_TIER2_WAVE1.length} · core9=${core9ok} · txFortBend=${txOk}`
);

if (passed !== FL_TIER2_WAVE1.length || !core9ok || !txOk) process.exit(1);
