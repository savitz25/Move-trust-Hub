/**
 * Locked Tier 2 contract checks for OH Wave 1.
 * Run: npx tsx scripts/verify-oh-tier2-wave1.ts
 */
import {
...
const FORBIDDEN =
  /\b(FDACS|Chapter 507|BHGS|TxDMV|NYSDOT|NJ BPU|Board of Public Utilities|GA DPS|MCCD|PA PUC|UTC household goods permit|NCUC)\b/i;

const failures: string[] = [];

for (const slug of OH_TIER2_WAVE1) {
  if ((OH_TIER1_CORE as readonly string[]).includes(slug)) {
    failures.push(`${slug}: listed in OH_TIER1_CORE — skip Tier 2 rebuild`);
  }

  const pack = getCountyIntelligencePack('ohio', slug);
  if (!pack) {
  ...
  const routes = getCountyPopularRoutes('ohio', slug);
  ...
  if (!/PUCO|Public Utilities Commission of Ohio|household-goods|household goods/i.test(blob)) {
    failures.push(`${slug}: missing OH PUCO / household-goods language`);
  }
  if (!/FMCSA/i.test(blob)) failures.push(`${slug}: missing FMCSA`);
  if (FORBIDDEN.test(blob)) failures.push(`${slug}: foreign regulator bleed`);
  if (pack.stateSlug !== 'ohio') failures.push(`${slug}: wrong stateSlug`);
...
// Warren name collision: ensure OH pack is registered (not NJ/NY)
const warren = getCountyIntelligencePack('ohio', 'warren');
if (!warren || warren.stateSlug !== 'ohio') {
  failures.push('warren: OH pack missing or wrong state');
}
if (warren && /New Jersey|New York|NJ |NY /i.test(JSON.stringify(warren.parentCompare))) {
  failures.push('warren: NJ/NY bleed in parent compare');
}

if (failures.length) {
  console.error('\nOH TIER 2 WAVE 1 CONTRACT FAILURES:');
  for (const f of failures) console.error(' -', f);
  process.exit(1);
}

console.log('\nOH Tier 2 Wave 1 contract checks passed.');
