/**
 * QA for Georgia Tier-1 Core 12 + TX/FL regression.
 * Run: npx tsx scripts/qa-ga-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'georgia') {
    checks.gaDpsLanguage =
      /department of public safety|mccd|motor carrier compliance/i.test(
        faq.map((f) => f.answer).join(' ')
      ) ||
      /department of public safety|mccd|dps/i.test(heroBlob);
    checks.noBhgsOnGeorgia = !/bhgs|bureau of household goods/i.test(heroBlob);
    checks.noTxdmvOnGeorgia = !/txdmv|department of motor vehicles/i.test(heroBlob);
    checks.noFdacsOnGeorgia = !/fdacs|chapter 507/i.test(heroBlob);
  }
...
const rows: Row[] = GA_TIER1_CORE12.map((slug) => auditCounty('georgia', slug));
rows.push(auditCounty('texas', 'harris'));
rows.push(auditCounty('florida', 'broward'));

const report = {
  generatedAt: new Date().toISOString(),
  gaCore12: GA_TIER1_CORE12.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  scaleRecommendation: rows.every((r) => r.pass)
    ? 'Georgia Core 12 Tier-1 complete on locked template. Next: onboard true GA local movers for local-first segments.'
    : 'Fix failing counties before calling GA Core 12 complete',
  rows,
};

writeFileSync('scripts/output/ga-tier1-qa.json', JSON.stringify(report, null, 2));
