/**
 * QA for Illinois Tier-1 Core 12.
 * Run: npx tsx scripts/qa-il-tier1.ts
 */
...
const FORBIDDEN_REG =
  /\b(BHGS|FDACS|NCUC|NYSDOT|Class E|TDOR|VA DMV|PA PUC|NJ public-mover|public-mover credentials|Georgia DPS|MCCD|TxDMV)\b/i;
...
function auditCounty(slug: string): Row {
  ...
  const faqText = county
    ? buildCountyFaqItems(county, 'Illinois', movers)
        .map((f) => f.answer)
        .join(' ')
    : '';
  ...
  checks.ilIccPresent = /Illinois Commerce Commission|\bICC\b|Household Goods license/i.test(
    blob
  );
  checks.fmcsaPresent = /FMCSA/i.test(blob);
  checks.noForeignRegulator =
    !FORBIDDEN_REG.test(blob) && !FORBIDDEN_REG.test(faqText);
  checks.ilStateSlug = pack.stateSlug === 'illinois';
...
const rows = IL_TIER1_CORE12.map((slug) => auditCounty(slug));
const passed = rows.filter((r) => r.pass).length;

const fairfax = getCountyIntelligencePack('virginia', 'fairfax');
const meck = getCountyIntelligencePack('north-carolina', 'mecklenburg');
const regression = {
  fairfax: Boolean(fairfax?.h1 && fairfax.zones.length >= 4),
  mecklenburg: Boolean(meck?.h1 && meck.zones.length >= 4),
};

const report = {
  ilCore12: IL_TIER1_CORE12.length,
  passed,
  failed: IL_TIER1_CORE12.length - passed,
  regression,
  rows,
};

writeFileSync('scripts/output/il-tier1-qa.json', JSON.stringify(report, null, 2));
console.log(JSON.stringify(report, null, 2));
const ok =
  passed === IL_TIER1_CORE12.length &&
  regression.fairfax &&
  regression.mecklenburg;
console.log(
  ok ? 'IL TIER1 QA PASS' : `IL TIER1 QA FAIL ${passed}/${IL_TIER1_CORE12.length}`
);
process.exit(ok ? 0 : 1);
