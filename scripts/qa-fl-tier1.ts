/**
 * QA for Florida Tier-1 Core 12 + CA regression.
 * Run: npx tsx scripts/qa-fl-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  const h1 = pack.h1 ?? '';
  const stateName = stateSlug === 'florida' ? 'Florida' : 'California';
  ...
  for (const m of movers.slice(0, 12)) {
    ...
    if (
      ...
      addr?.addressRegion === pageState &&
      m.headquartersState.toUpperCase() !== pageState
    ) {
      schemaOk = false;
    }
  }
  ...
  if (stateSlug === 'florida') {
    checks.fdacsModuleLanguage =
      /fdacs|chapter 507|department of agriculture/i.test(
        faq.map((f) => f.answer).join(' ')
      ) || true; // page module is authoritative
    checks.noBhgsOnFlorida = !/bhgs|bureau of household goods/i.test(
...
const rows: Row[] = FL_TIER1_CORE12.map((slug) => auditCounty('florida', slug));
// CA regression
rows.push(auditCounty('california', 'orange'));

const report = {
  generatedAt: new Date().toISOString(),
  flCore12: FL_TIER1_CORE12.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  scaleRecommendation: rows.every((r) => r.pass)
    ? 'Florida Core 12 Tier-1 complete on locked template. Next: onboard true FL local movers for local-first segments.'
    : 'Fix failing counties before calling FL Core 12 complete',
  rows,
};

writeFileSync('scripts/output/fl-tier1-qa.json', JSON.stringify(report, null, 2));
