/**
 * Locked Tier 2 contract checks for MI Wave 1.
 * Run: npx tsx scripts/verify-mi-tier2-wave1.ts
 */
import {
...
const FORBIDDEN =
  /\b(FDACS|Chapter 507|BHGS|TxDMV|NYSDOT|NJ BPU|Board of Public Utilities|PUCO|GA DPS|MCCD|PA PUC|NCUC|Virginia DMV)\b/i;

const failures: string[] = [];

for (const slug of MI_TIER2_WAVE1) {
  if ((MI_TIER1_CORE as readonly string[]).includes(slug)) {
    failures.push(`${slug}: listed in MI_TIER1_CORE — skip Tier 2 rebuild`);
  }

  const pack = getCountyIntelligencePack('michigan', slug);
  if (!pack) {
  ...
  const routes = getCountyPopularRoutes('michigan', slug);
  ...
  if (!/MSP|CVED|Motor Carrier|household goods|household-goods/i.test(blob)) {
    failures.push(`${slug}: missing MI MSP CVED / household-goods language`);
  }
  if (!/FMCSA/i.test(blob)) failures.push(`${slug}: missing FMCSA`);
  if (FORBIDDEN.test(blob)) failures.push(`${slug}: foreign regulator bleed`);
  if (pack.stateSlug !== 'michigan') failures.push(`${slug}: wrong stateSlug`);
...
const monroe = getCountyIntelligencePack('michigan', 'monroe');
if (!monroe || monroe.stateSlug !== 'michigan') {
  failures.push('monroe: MI pack missing or wrong state');
}

if (failures.length) {
  console.error('\nMI TIER 2 WAVE 1 CONTRACT FAILURES:');
  for (const f of failures) console.error(' -', f);
  process.exit(1);
}

console.log('\nMI Tier 2 Wave 1 contract checks passed.');
