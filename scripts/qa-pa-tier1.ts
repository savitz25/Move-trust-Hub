/**
 * QA for Pennsylvania Tier-1 Core 12 + VA/NC regression.
 * Run: npx tsx scripts/qa-pa-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  checks.stateCredentialPhrase = faqText.includes(
    stateIntrastateCredentialPhrase(stateSlug).slice(0, 10)
  );
  ...
  if (stateSlug === 'pennsylvania') {
    checks.pucLanguage = /puc|public utility commission/i.test(heroBlob + faqText);
    checks.noForeignRegulators = !/ncuc|fdacs|txdmv|nysdot|bhgs|tdor|certificate of fitness/i.test(
...
const rows: Row[] = PA_TIER1_CORE12.map((slug) => auditCounty('pennsylvania', slug));
rows.push(auditCounty('virginia', 'fairfax'));
rows.push(auditCounty('north-carolina', 'mecklenburg'));

const paH1s = rows.filter((r) => r.county.startsWith('pennsylvania/')).map((r) => r.h1);
const h1Distinct = new Set(paH1s).size === paH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  paCore12: PA_TIER1_CORE12.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossPa: h1Distinct,
  scaleRecommendation: rows.every((r) => r.pass) && h1Distinct
    ? 'Pennsylvania Core 12 Tier-1 complete on locked template. Next: onboard true PA local movers for local-first segments.'
    : 'Fix failing counties before calling PA Core 12 complete',
  summaryTable: rows
    .filter((r) => r.county.startsWith('pennsylvania/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  weakLocalCounties: rows
    .filter((r) => r.county.startsWith('pennsylvania/') && r.localCount === 0)
    .map((r) => r.county),
  rows,
};

mkdirSync('scripts/output', { recursive: true });
writeFileSync('scripts/output/pa-tier1-qa.json', JSON.stringify(report, null, 2));
