/**
 * QA for Ohio Tier-1 Core 12 + PA/VA regression.
 * Run: npx tsx scripts/qa-oh-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'ohio') {
    checks.pucoLanguage = /puco|public utilities commission of ohio/i.test(heroBlob + faqText);
    checks.noForeignRegulators = !/ncuc|fdacs|txdmv|nysdot|bhgs|tdor|pa puc|icc household/i.test(
...
const rows: Row[] = OH_TIER1_CORE12.map((slug) => auditCounty('ohio', slug));
rows.push(auditCounty('pennsylvania', 'philadelphia'));
rows.push(auditCounty('virginia', 'fairfax'));

const ohH1s = rows.filter((r) => r.county.startsWith('ohio/')).map((r) => r.h1);
const h1Distinct = new Set(ohH1s).size === ohH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  ohCore12: OH_TIER1_CORE12.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossOh: h1Distinct,
  scaleRecommendation: rows.every((r) => r.pass) && h1Distinct
    ? 'Ohio Core 12 Tier-1 complete on locked template. Next: onboard true OH local movers for local-first segments.'
    : 'Fix failing counties before calling OH Core 12 complete',
  summaryTable: rows
    .filter((r) => r.county.startsWith('ohio/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  weakLocalCounties: rows
    .filter((r) => r.county.startsWith('ohio/') && r.localCount === 0)
    .map((r) => r.county),
  rows,
};

mkdirSync('scripts/output', { recursive: true });
writeFileSync('scripts/output/oh-tier1-qa.json', JSON.stringify(report, null, 2));
