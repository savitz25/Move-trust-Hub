/**
 * QA IL Tier 2 Wave 1 packs against locked contract.
 * Run: npx tsx --tsconfig tsconfig.json scripts/qa-il-tier2-wave1.ts
 */
import {
...
const core = new Set(IL_TIER1_CORE12 as readonly string[]);
const wave1 = [...IL_TIER2_WAVE1];
...
for (const slug of wave1) {
  const p = getCountyIntelligencePack('illinois', slug);
  if (!p) {
  ...
  const routes = getCountyPopularRoutes('illinois', slug);
  const issues: string[] = [];
  ...
  if (!blob.includes('ICC') && !blob.includes('Commerce Commission'))
    issues.push('no-ICC');
  ...
  if (
    /NJ BPU|NYSDOT|FDACS|TxDMV|BHGS|NCUC|PA PUC|Public Utility Commission \(PUC\)/i.test(
      blob
    )
  )
...
for (const slug of ['cook', 'dupage', 'will', 'kane', 'winnebago']) {
  const p = getCountyIntelligencePack('illinois', slug);
  if (!p) {
