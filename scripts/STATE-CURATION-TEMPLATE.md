...
| Washington | 1/39 (expanding) | 10 | 10 (King) | `king` 0.85 | `count-washington-movers.ts`, `apply-washington-mover-expansion.ts`, `audit-washington-seo.ts` |

> **Washington note:** Washington has **39 counties** — curation is **expanding statewide**, starting with **King County** (Seattle metro). **King** uses Two Men and a Truck franchise #1; additional Puget Sound and regional counties will follow. Remaining counties keep generated `washington-region-*` metro fallback until curated. Nearby links include Snohomish, Pierce, Kitsap, plus long-distance Alaska and Hawaii hubs. Add to `SKIP_STATES` and `CURATED_STATE_SLUGS`.
