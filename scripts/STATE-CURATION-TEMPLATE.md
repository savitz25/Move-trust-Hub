...
| Washington | 8/39 (expanding) | 10 | 10 (curated counties) | `king`, `pierce`, `snohomish`, `spokane` 0.85 | `count-washington-movers.ts`, `apply-washington-mover-expansion.ts`, `audit-washington-seo.ts` |

> **Washington note:** Washington has **39 counties** — curation is **expanding statewide**. **8 premium hubs** curated: **King** (Seattle), **Pierce** (Tacoma), **Snohomish** (Everett), **Spokane**, **Clark** (Vancouver), **Thurston** (Olympia), **Kitsap**, and **Yakima** — 10 movers each. Franchise #1 in King, Pierce, Snohomish, Spokane, and Clark; `regional-*-wa-movers` in Thurston, Kitsap, and Yakima. Remaining counties keep generated `washington-region-*` metro fallback. Cross-county nearby links among curated Puget Sound and regional hubs plus Alaska long-distance links. Add to `SKIP_STATES` and `CURATED_STATE_SLUGS`.
