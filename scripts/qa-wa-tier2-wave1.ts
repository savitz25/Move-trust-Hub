/**
 * QA WA Tier 2 Wave 1 packs against locked contract.
 * Run: npx tsx --tsconfig tsconfig.json scripts/qa-wa-tier2-wave1.ts
 */
import {
...
const core = new Set(WA_TIER1_CORE10 as readonly string[]);
const wave1 = [...WA_TIER2_WAVE1];
...
for (const slug of wave1) {
  const p = getCountyIntelligencePack('washington', slug);
  if (!p) {
  ...
  const routes = getCountyPopularRoutes('washington', slug);
  const issues: string[] = [];
  ...
  if (!blob.includes('UTC') && !blob.includes('Utilities and Transportation'))
    issues.push('no-UTC');
  ...
  if (
    /NJ BPU|NYSDOT|NCUC household|Illinois Commerce|Florida FDACS|California BHGS|TxDMV|Colorado PUC|Arizona Corporation/i.test(
      blob
    )
  )
...
for (const slug of ['king', 'pierce', 'clark', 'whatcom', 'snohomish']) {
  const p = getCountyIntelligencePack('washington', slug);
  if (!p) {
