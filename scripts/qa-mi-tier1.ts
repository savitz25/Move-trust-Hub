/**
 * QA for Michigan Tier-1 Core 10 + OH Franklin / IL Cook regression.
 * Run: npx tsx scripts/qa-mi-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'michigan') {
    checks.miRegulatoryLanguage =
      /msp cved|michigan motor carrier|motor carrier act/i.test(heroBlob + faqText + packBlob);
    checks.noForeignRegulators = !/puco|ncuc|fdacs|txdmv|nysdot|bhgs|tdor|pa puc|icc household|colorado puc|utc household|nj public-mover/i.test(
...
const rows: Row[] = MI_TIER1_CORE10.map((slug) => auditCounty('michigan', slug));
rows.push(auditCounty('ohio', 'franklin'));
rows.push(auditCounty('illinois', 'cook'));

const miH1s = rows.filter((r) => r.county.startsWith('michigan/')).map((r) => r.h1);
const h1Distinct = new Set(miH1s).size === miH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  miCore10: MI_TIER1_CORE10.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossMi: h1Distinct,
  scaleRecommendation: rows.every((r) => r.pass) && h1Distinct
    ? 'Michigan Core 10 Tier-1 complete on locked template. Next: onboard true MI local movers for local-first segments.'
    : 'Fix failing counties before calling MI Core 10 complete',
  summaryTable: rows
    .filter((r) => r.county.startsWith('michigan/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  weakLocalCounties: rows
    .filter((r) => r.county.startsWith('michigan/') && r.localCount === 0)
    .map((r) => r.county),
  rows,
};

mkdirSync('scripts/output', { recursive: true });
writeFileSync('scripts/output/mi-tier1-qa.json', JSON.stringify(report, null, 2));
