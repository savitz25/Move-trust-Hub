/**
 * QA for Oregon Tier-1 Core 8 + WA King / MI Wayne regression.
 * Run: npx tsx scripts/qa-or-tier1.ts
 */
...
function auditCounty(stateSlug: string, slug: string): Row {
  ...
  if (stateSlug === 'oregon') {
    checks.odotLanguage = /odot|ors 825|commerce and compliance|household goods certificate/i.test(
      heroBlob + faqText + packBlob
    );
    checks.noForeignRegulators = !/utc household|puco|ncuc|fdacs|txdmv|nysdot|bhgs|tdor|pa puc|icc household|colorado puc|msp cved|nj public-mover|wa utc|dpu operating/i.test(
      `${pack.heroCredibility} ${pack.heroOpener}`
    );
    // Oregon's Washington County must remain OR identity (Beaverton/Hillsboro), not WA-state clone.
    // Explicit contrast language ("not King County") is allowed and expected.
    checks.notWashingtonStateBleed =
      pack.countySlug !== 'washington' ||
      (/beaverton|hillsboro|silicon forest|oregon/i.test(pack.heroOpener ?? '') &&
        !/^Moving in King County/i.test(pack.h1 ?? ''));
  }
...
const rows: Row[] = OR_TIER1_CORE8.map((slug) => auditCounty('oregon', slug));
rows.push(auditCounty('washington', 'king'));
rows.push(auditCounty('michigan', 'wayne'));

const orH1s = rows.filter((r) => r.county.startsWith('oregon/')).map((r) => r.h1);
const h1Distinct = new Set(orH1s).size === orH1s.length;

const report = {
  generatedAt: new Date().toISOString(),
  orCore8: OR_TIER1_CORE8.length,
  passCount: rows.filter((r) => r.pass).length,
  failCount: rows.filter((r) => !r.pass).length,
  h1DistinctAcrossOr: h1Distinct,
  scaleRecommendation: rows.every((r) => r.pass) && h1Distinct
    ? 'Oregon Core 8 Tier-1 complete on locked template. Next: onboard true OR local movers for local-first segments where weak.'
    : 'Fix failing counties before calling OR Core 8 complete',
  summaryTable: rows
    .filter((r) => r.county.startsWith('oregon/'))
    .map((r) => ({
      county: r.county,
      local: r.localCount,
  ...
  weakLocalCounties: rows
    .filter((r) => r.county.startsWith('oregon/') && r.localCount === 0)
    .map((r) => r.county),
  rows,
};

mkdirSync('scripts/output', { recursive: true });
writeFileSync('scripts/output/or-tier1-qa.json', JSON.stringify(report, null, 2));
