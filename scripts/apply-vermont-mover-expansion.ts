/**
 * One-shot script: ensure VT curated assignments meet county mover targets.
 * Run: npx tsx scripts/apply-vermont-mover-expansion.ts
 */
...
const assignmentsPath = 'data/vermont-county-assignments.ts';
const file = readFileSync(assignmentsPath, 'utf8');
const match = file.match(/const CURATED_VT_COUNTIES[^=]*=\s*(\{[\s\S]*?\n\});/);
if (!match) throw new Error('Could not parse CURATED_VT_COUNTIES');

const current: Record<string, string[]> = eval(`(${match[1]})`);

function poolIdForSlug(slug: string): string {
  return slug === 'grand-isle' ? 'grand-isle-metro-vt' : `${slug}-metro-vt`;
}
...
function expandIds(slug: string, existing: string[]): string[] {
  ...
  const vtMoverIds = Object.keys(fullMoversCatalog).filter((id) => id.endsWith('-vt'));
  for (const id of vtMoverIds) {
    if (ids.length >= DEFAULT_TARGET) break;
...
for (const slug of Object.keys(vermontCountyResearch)) {
...
const block = `const CURATED_VT_COUNTIES: Record<string, string[]> = {\n${lines.join('\n')}\n};`;
const updated = file.replace(/const CURATED_VT_COUNTIES[^=]*=\s*\{[\s\S]*?\n\};/, block);
