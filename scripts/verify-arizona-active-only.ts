...
async function main() {
  ...
  for (const row of arizonaCountyMoverAssignments) {
  ...
  for (const c of counties) {
    const r = getMoversForCounty('arizona', c.slug);
  ...
  for (const slug of [
    ...
    const r = getMoversForCounty('arizona', slug);
    const assigned =
      arizonaCountyMoverAssignments.find((a) => a.countySlug === slug)
        ?.moverIds ?? [];
    console.log(
      `${slug}: assigned=${assigned.length} listed=${r?.movers.length} top=${r?.movers
        .slice(0, 4)
        .map((m) => m.name)
        .join(' | ')}`
    );
