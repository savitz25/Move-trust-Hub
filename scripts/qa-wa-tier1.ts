/**
 * QA for Washington Tier-1 Core 10.
 * Run: npx tsx scripts/qa-wa-tier1.ts
 */
...
const FORBIDDEN_REG =
  /\b(BHGS|FDACS|NCUC|NYSDOT|Class E|TDOR|VA DMV|PA PUC|NJ public-mover|public-mover credentials|Georgia DPS|MCCD|TxDMV|IL ICC|PUCO|Colorado PUC HHG|Title 40, Article 10\.1)\b/i;
...
function auditCounty(slug: string): Row {
  ...
  const faqText = county
    ? buildCountyFaqItems(county, 'Washington', movers)
        .map((f) => f.answer)
        .join(' ')
    : '';
  ...
  checks.waUtcPresent =
    /Utilities and Transportation Commission|\bUTC\b|household goods permit/i.test(
      blob
    );
  checks.fmcsaPresent = /FMCSA/i.test(blob);
  checks.noForeignRegulator =
    !FORBIDDEN_REG.test(blob) && !FORBIDDEN_REG.test(faqText);
  checks.waStateSlug = pack.stateSlug === 'washington';
...
const rows = WA_TIER1_CORE10.map((slug) => auditCounty(slug));
const passed = rows.filter((r) => r.pass).length;

const denver = getCountyIntelligencePack('colorado', 'denver');
const franklin = getCountyIntelligencePack('ohio', 'franklin');
const regression = {
  denver: Boolean(denver?.h1 && denver.zones.length >= 4),
  franklin: Boolean(franklin?.h1 && franklin.zones.length >= 4),
};

const report = {
  waCore10: WA_TIER1_CORE10.length,
  passed,
  failed: WA_TIER1_CORE10.length - passed,
  regression,
  rows,
};

writeFileSync('scripts/output/wa-tier1-qa.json', JSON.stringify(report, null, 2));
console.log(JSON.stringify(report, null, 2));
const ok =
  passed === WA_TIER1_CORE10.length &&
  regression.denver &&
  regression.franklin;
console.log(
  ok ? 'WA TIER1 QA PASS' : `WA TIER1 QA FAIL ${passed}/${WA_TIER1_CORE10.length}`
);
process.exit(ok ? 0 : 1);
