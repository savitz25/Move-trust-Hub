/**
 * QA NC Tier 2 Wave 1 packs against locked contract.
 * Run: npx tsx --tsconfig tsconfig.json scripts/qa-nc-tier2-wave1.ts
 */
import {
...
const core = new Set(NC_TIER1_CORE12 as readonly string[]);
const wave1 = [...NC_TIER2_WAVE1];
...
for (const slug of wave1) {
  const p = getCountyIntelligencePack('north-carolina', slug);
  if (!p) {
  ...
  const routes = getCountyPopularRoutes('north-carolina', slug);
  const issues: string[] = [];
  ...
  if (!blob.includes('NCUC') && !blob.includes('Utilities Commission'))
    issues.push('no-NCUC');
  ...
  if (/NJ BPU|NYSDOT|FDACS|TxDMV|BHGS|PA PUC|Public Utility Commission \(PUC\)/i.test(blob))
...
for (const slug of ['mecklenburg', 'wake', 'guilford', 'durham', 'new-hanover']) {
  const p = getCountyIntelligencePack('north-carolina', slug);
  if (!p) {
