/**
 * Full SEO / E-E-A-T audit for Wyoming local movers directory.
 * Run: npx tsx scripts/audit-wyoming-seo.ts
 */
import { wyomingCountyResearch } from '../data/wyoming-county-research';
import { getWyomingCountyTestimonials } from '../data/wyoming-county-testimonials';
import { getWyomingNearbyCounties } from '../lib/local-movers/wyoming-nearby';
import { getCounty } from '../lib/local-movers/geography/index';
import { getMoversForCounty } from '../lib/local-movers/index';

const PREMIUM_TARGETS: Record<string, number> = {
  laramie: 10,
  natrona: 9,
  teton: 8,
};
const DEFAULT_TARGET = 5;
const EXPECTED_COUNT = 23;

function getTarget(slug: string): number {
  return PREMIUM_TARGETS[slug] ?? DEFAULT_TARGET;
}

const issues: string[] = [];
const curatedSlugs = Object.keys(wyomingCountyResearch).sort();

for (const slug of curatedSlugs) {
  const target = getTarget(slug);
  const c = getCounty('wyoming', slug);
  const result = getMoversForCounty('wyoming', slug);
  const moverCount = result?.movers.length ?? 0;

  if (moverCount < target) {
    issues.push(`movers<${target}: ${slug} (${moverCount})`);
  }
  if (!wyomingCountyResearch[slug]) {
    issues.push(`no research: ${slug}`);
  }
  if (getWyomingCountyTestimonials(slug).length < 3) {
    issues.push(`testimonials<3: ${slug}`);
  }
  if (getWyomingNearbyCounties(slug).length < 3) {
    issues.push(`nearby<3: ${slug}`);
  }
  if (result?.isRegionalFallback) {
    issues.push(`regional fallback: ${slug}`);
  }
  if (!c?.seat) {
    issues.push(`missing seat: ${slug}`);
  }
}

console.log('Wyoming SEO audit');
console.log('==================');
console.log(`Curated counties: ${curatedSlugs.length}`);
const researchCount = Object.keys(wyomingCountyResearch).length;
console.log(`Research entries: ${researchCount}`);
if (researchCount !== EXPECTED_COUNT) {
  issues.push(`research count: ${researchCount} (expected ${EXPECTED_COUNT})`);
}

for (const slug of curatedSlugs) {
  const target = getTarget(slug);
  const n = getMoversForCounty('wyoming', slug)?.movers.length ?? 0;
  console.log(
    `  ${slug}: ${n} movers (target ${target}), testimonials ${getWyomingCountyTestimonials(slug).length}, nearby ${getWyomingNearbyCounties(slug).length}`
  );
}

console.log(`Issues: ${issues.length}`);

if (issues.length === 0) {
  console.log(`\n✓ Wyoming curated counties meet full curation standard (${EXPECTED_COUNT}/${EXPECTED_COUNT}).`);
} else {
  console.log('\nIssues:');
  for (const line of issues) console.log(`  ${line}`);
  process.exit(1);
}