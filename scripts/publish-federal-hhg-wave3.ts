/**
 * Publish Task 008A Wave 3 from the Task 007 candidate manifest only.
 * Carriers only. Never inserts IDENTITY_REVIEW_REQUIRED, INACTIVE, or non-manifest USDOTs.
 * Never calls Google Places. Rollback isolates Wave 3 from Waves 1 and 2.
 *
 * npm run publish:federal-hhg-wave3 -- --dry-run --limit 250
 * npm run publish:federal-hhg-wave3 -- --limit 250
 * npm run publish:federal-hhg-wave3 -- --limit 1279
 * npm run publish:federal-hhg-wave3 -- --indexable
 * npm run publish:federal-hhg-wave3 -- --rollback
 */
...
const MANIFEST_PATH = resolve(process.cwd(), 'docs/task-007-wave3-selection.json');
...
async function main() {
  ...
  const limit = Math.min(argNum('--limit', 250), 1279);
  ...
  if (rollback) {
    ...
    const wave2After = await client.query(
      `SELECT count(*)::int AS n FROM public.federal_hhg_wave_publication
        WHERE wave_id = $1 AND status <> 'unpublished'`,
      [WAVE_2_PUBLICATION_ID]
    );
      ...
      JSON.stringify(
        {
          rollback: true,
          dry,
          wave3: WAVE_3_PUBLICATION_ID,
          wave1_before: wave1.rows[0],
          wave1_after: wave1After.rows[0],
          wave2_after: wave2After.rows[0],
  ...
  for (const candidate of manifest.candidates) {
    ...
    const check = revalidateWave3Candidate(stagedRow, manifestDots, existingDots);
    if (!check.ok) {
  ...
  const selectedMeta =
    limit <= 250 && alreadyUsdots.size === 0
      ? selectWave2Canary(eligibleMeta, { limit: remaining, maxBrokers: 0 })
  ...
  writeFileSync(
    resolve(docsDir, 'task-008a-wave3-publish-selection.json'),
    JSON.stringify(
      {
        google_places_requests: 0,
        wave_id: WAVE_3_PUBLICATION_ID,
        excluded,
  ...
  writeFileSync(resolve(docsDir, 'task-008a-wave3-publish.json'), JSON.stringify(report, null, 2) + '\n');
