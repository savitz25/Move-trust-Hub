...
async function main() {
  ...
  const active = (companies as CompanyRow[]).filter((c) => !c.out_of_service);
  console.log(`Active companies (not OOS): ${active.length}`);
...
/**
 * Active directory companies from Supabase (not out_of_service).
 * Generated by: npx tsx scripts/rebuild-texas-from-active-db.ts
 * Count: ${displayable.length} displayable of ${active.length} active.
 *
 * Merged into fullMoversCatalog so Texas (and future) county pages can list
 * only real active companies — never deactivated seed/template movers.
 */
