/**
 * One-shot script: ensure AK curated assignments meet borough mover targets.
 * Run: npx tsx scripts/apply-alaska-mover-expansion.ts
 */
...
const assignmentsPath = 'data/alaska-county-assignments.ts';
const file = readFileSync(assignmentsPath, 'utf8');
const match = file.match(/const CURATED_AK_COUNTIES[^=]*=\s*(\{[\s\S]*?\n\});/);
if (!match) throw new Error('Could not parse CURATED_AK_COUNTIES');

const current: Record<string, string[]> = eval(`(${match[1]})`);

function poolIdForSlug(slug: string): string {
  return `${slug}-metro-ak`;
}
...
function expandIds(slug: string, existing: string[]): string[] {
  ...
  const akMoverIds = Object.keys(fullMoversCatalog).filter((id) => id.endsWith('-ak'));
  for (const id of akMoverIds) {
    if (ids.length >= DEFAULT_TARGET) break;
...
for (const slug of Object.keys(alaskaCountyResearch)) {
...
const block = `const CURATED_AK_COUNTIES: Record<string, string[]> = {\n${lines.join('\n')}\n};`;
const updated = file.replace(/const CURATED_AK_COUNTIES[^=]*=\s*\{[\s\S]*?\n\};/, block);
