/**
 * Locked Tier 2 contract checks for GA Wave 1.
 * Run: npx tsx scripts/verify-ga-tier2-wave1.ts
 */
import {
...
for (const slug of GA_TIER2_WAVE1) {
...
if (failures.length) {
  console.error('\nGA TIER 2 WAVE 1 CONTRACT FAILURES:');
  for (const f of failures) console.error(' -', f);
  process.exit(1);
}

console.log('\nGA Tier 2 Wave 1 contract checks passed.');
