/**
 * Locked Tier 2 contract checks for VA Wave 1.
 * Run: npx tsx scripts/verify-va-tier2-wave1.ts
 */
import {
...
const FORBIDDEN =
  /\b(FDACS|Chapter 507|BHGS|TxDMV|NYSDOT|NJ BPU|Board of Public Utilities|PUCO|GA DPS|MCCD|PA PUC|NCUC)\b/i;

const failures: string[] = [];

for (const slug of VA_TIER2_WAVE1) {
...
const montgomery = getCountyIntelligencePack('virginia', 'montgomery');
if (!montgomery || montgomery.stateSlug !== 'virginia') {
  failures.push('montgomery: VA pack missing');
}
const frederick = getCountyIntelligencePack('virginia', 'frederick');
if (!frederick || frederick.stateSlug !== 'virginia') {
  failures.push('frederick: VA pack missing');
}

if (failures.length) {
  console.error('\nVA TIER 2 WAVE 1 CONTRACT FAILURES:');
  for (const f of failures) console.error(' -', f);
  process.exit(1);
}

console.log('\nVA Tier 2 Wave 1 contract checks passed.');
