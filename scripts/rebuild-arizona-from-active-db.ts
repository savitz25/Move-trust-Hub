/**
 * Arizona-only cleanup + rebuild using active Supabase companies.
 * Same process as Texas / California.
 *
 * Active = not out_of_service AND authority_active !== false
 * Per county order: local (destination) → AZ-focused → national
 *
 * Run: npx tsx scripts/rebuild-arizona-from-active-db.ts
 */
import {
  readFileSync,
  writeFileSync,
...
function isArizonaHq(c: CompanyRow): boolean {
  const hq = (c.headquarters || '').toLowerCase();
  return (
    /,\s*az\b/.test(hq) ||
    hq.endsWith(' arizona') ||
    /\barizona\b/.test(hq) ||
    /^(phoenix|tucson|mesa|chandler|scottsdale|glendale|gilbert|tempe|peoria|surprise|yuma|avondale|goodyear|flagstaff|buckeye|casa grande|lake havasu|maricopa|sierra vista|prescott|bullhead city|apache junction|maricopa|queen creek|san tan valley|oro valley|prescott valley|kingman|florence|nogales|douglas|show low|payson|sedona|page|winslow|cottonwood|camp verde|safford|globe|bisbee|st johns|holbrook|willcox|eloy|coolidge|somerton|paradises valley|fountain hills|paradise valley)\b/i.test(
      hq
    )
  );
}
...
async function main() {
  ...
  console.log(
    `AZ dest dropped (non-active): ${destDropped}; AZ-dest companies: ${stateWideSlugs.size}; dest rows: ${(destRows ?? []).length}`
  );

  for (const c of displayable) {
    if (isArizonaHq(c)) stateWideSlugs.add(c.slug);
  }

  const nationalSlugs = new Set(
    displayable.filter(isNational).map((c) => c.slug)
  );
  console.log(`National: ${nationalSlugs.size}; AZ-focused: ${stateWideSlugs.size}`);

  // Refresh shared active directory catalog (same as CA/TX)
...
`
  ...
  writeFileSync(
    'data/arizona-county-assignments.ts',
...
`
  ...
  console.log('Wrote data/arizona-county-assignments.ts');

  mkdirSync('scripts/output', { recursive: true });
  writeFileSync(
    'scripts/output/az-active-rebuild-summary.json',
    JSON.stringify(
      {
