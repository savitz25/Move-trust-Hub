/**
 * Startup environment variable validation for Move Trust Hub.
 * Called at build/startup via instrumentation.ts.
 */
...
export function validateEnv(options?: { strict?: boolean }): EnvCheckResult {
  ...
  if (!ok) {
    const msg = `[Move Env] Missing required variables: ${missing.join(', ')}`;
    if (strict) throw new Error(msg);
    console.warn(msg);
  }

  if (warnings.length && process.env.NODE_ENV === 'development') {
    console.info('[Move Env] Warnings:', warnings.join('; '));
  }
