...
const mocks = {
  ...
  '@/lib/supabase/server': `export async function createClient(){state.clients++;return {from(table){state.table=table;return {
    ...
    select(columns){state.columns=columns;return {async eq(column,value){state.readOwner={column,value};return {data:[{company_slug:'b3-test-mover'}],error:null}}}}
  }}}}`,
...
const result = await build({
  stdin: { contents: "export { saveMoverAction, getSavedMoverSlugsAction } from './actions/save-my-move';", resolveDir: process.cwd() },
...
function fixture(id) {
  ...
  return {state,save:module.exports.saveMoverAction,read:module.exports.getSavedMoverSlugsAction};
}
...
test('QA-M9 mocked: account confirmation reads only the verified expected owner',async()=>{
  const {state,read}=fixture('owner-a');
  assert.equal(JSON.stringify(await read('owner-a')), '["b3-test-mover"]');
  assert.equal(state.table,'saved_movers');
  assert.equal(state.columns,'company_slug');
  assert.equal(state.readOwner.column,'user_id');
  assert.equal(state.readOwner.value,'owner-a');
  assert.equal(state.rows.length+state.profiles+state.activities,0);
});
test('QA-M9 mocked: mismatched owner cannot return another account confirmation',async()=>{
  for (const id of [null,'owner-b']) {
    const {state,read}=fixture(id);
    assert.equal(JSON.stringify(await read('owner-a')), '[]');
