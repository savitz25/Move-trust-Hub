...
export const NE_SECTION_ORDER: CountyIntelligenceSectionId[] = [
  'whatMakesDifferent',
  'zones',
  'costDrivers',
  'seasonal',
...
export const NE_COMMON_RESOURCES: CountyIntelligenceResource[] = [
  {
    label: 'Nebraska PSC — Household goods movers licensees',
    href: 'https://psc.nebraska.gov/household-goods-movers-licensees',
    external: true,
    note: 'Intrastate Household Goods Mover License list',
  },
  {
    label: 'Nebraska Public Service Commission',
    href: 'https://psc.nebraska.gov/',
    external: true,
    note: 'State household goods licensing authority',
  },
  {
  ...
  {
    label: 'All Nebraska local mover guides',
    href: '/local-movers/nebraska',
  },
];

export function finalizeNePack(
  pack: Omit<
    CountyIntelligencePack,
  ...
  const merged = [
    ...localResources,
    ...NE_COMMON_RESOURCES.filter((r) => !seen.has(r.href)),
  ];
  ...
  return {
    ...
    sectionOrder: pack.sectionOrder ?? NE_SECTION_ORDER,
    resources: {
