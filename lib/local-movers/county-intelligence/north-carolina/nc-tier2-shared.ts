...
/**
 * North Carolina Tier 2 content contract (LOCKED — same shell as CA/TX/FL/NJ/NY/PA/GA Tier 2).
 *
 * Intrastate: North Carolina Utilities Commission (NCUC) household-goods
 * mover certification (C-#) for pure in-state North Carolina moves.
 * Interstate: FMCSA USDOT/MC.
 * Do NOT hardcode PA PUC, NJ BPU, NYSDOT, FDACS, TxDMV, BHGS, or other-state credentials.
 */

export const NC_TIER2_REG_BULLET = {
  title: 'North Carolina NCUC household goods vs interstate FMCSA',
  detail:
    'Moves entirely within North Carolina are generally subject to North Carolina Utilities Commission (NCUC) household-goods mover certification (C-#) and consumer protections for intrastate household goods transportation. Match the legal name on the estimate to the carrier you hired. Any out-of-state leg (South Carolina, Virginia, Tennessee, Georgia, or beyond) needs active FMCSA USDOT (and usually MC) authority — verify on FMCSA SAFER. A USDOT alone is not North Carolina intrastate permission, and NCUC certification alone does not authorize interstate delivery.',
} as const;

export const NC_TIER2_COMMON_RESOURCES: CountyIntelligenceResource[] = [
  {
...
export const NC_TIER2_DEFAULT_LAST_REVIEWED = '2026-07-24';

export type NcTier2PackInput = Omit<
  CountyIntelligencePack,
...
/** Wrap a North Carolina Tier 2 pack with locked contract defaults. */
export function finalizeNcTier2Pack(pack: NcTier2PackInput): CountyIntelligencePack {
  ...
  if (zones.length < 2 || zones.length > 4) {
    console.warn(
      `[nc-tier2] ${pack.countySlug}: expected 2–4 zones, got ${zones.length}`
    );
  }
  if (specialized.length < 2 || specialized.length > 3) {
    console.warn(
      `[nc-tier2] ${pack.countySlug}: expected 2–3 specialized modules, got ${specialized.length}`
    );
  }
  if (!pack.parentCompare?.bullets?.length || pack.parentCompare.bullets.length < 3) {
    console.warn(
      `[nc-tier2] ${pack.countySlug}: parentCompare needs ≥3 concrete bullets`
    );
  }
  if (pack.h1 && !/^Moving in /i.test(pack.h1)) {
    console.warn(
      `[nc-tier2] ${pack.countySlug}: H1 should be role-forward “Moving in …” (got: ${pack.h1})`
    );
  }
  if (pack.h1 && /Movers Serving/i.test(pack.h1)) {
    console.warn(
      `[nc-tier2] ${pack.countySlug}: H1 must not use “Movers Serving…”`
    );
  }
  ...
  const merged = [
    ...localResources,
    ...NC_TIER2_COMMON_RESOURCES.filter((r) => !seen.has(r.href)),
  ];
  ...
  const lastReviewed =
    ...
    !Number.isNaN(Date.parse(pack.lastReviewed))
      ? pack.lastReviewed
      : NC_TIER2_DEFAULT_LAST_REVIEWED;
  ...
  return {
    ...
    resources: {
      ...
      intro:
        ...
        'Official links first; directory listings are independent, not endorsements. Verify NCUC household-goods certification for in-state North Carolina moves and FMCSA for interstate legs before deposits.',
      items: merged,
    },
  };
}
