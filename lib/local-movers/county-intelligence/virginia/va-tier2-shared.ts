...
/**
 * Virginia Tier 2 content contract (LOCKED — same shell as CA/TX/FL/NJ/NY/PA/GA/NC/OH).
 *
 * Intrastate: Virginia DMV Motor Carrier Services household goods / Certificate of Fitness
 * frameworks (distance can change the framework).
 * Interstate: FMCSA USDOT/MC.
 * Do NOT hardcode FDACS, NJ BPU, NYSDOT, PUCO, GA DPS, BHGS, TxDMV, or other-state credentials.
 */

export const VA_TIER2_REG_BULLET = VA_REG_BULLET;

export const VA_TIER2_COMMON_RESOURCES: CountyIntelligenceResource[] = VA_COMMON_RESOURCES;

export type VaTier2PackInput = Omit<
  CountyIntelligencePack,
  | 'stateSlug'
...
/** Wrap a Virginia Tier 2 pack with locked contract defaults. */
export function finalizeVaTier2Pack(pack: VaTier2PackInput): CountyIntelligencePack {
  ...
  if (zones.length < 2 || zones.length > 4) {
    console.warn(
      `[va-tier2] ${pack.countySlug}: expected 2–4 zones, got ${zones.length}`
    );
  }
  if (specialized.length < 2 || specialized.length > 3) {
    console.warn(
      `[va-tier2] ${pack.countySlug}: expected 2–3 specialized modules, got ${specialized.length}`
    );
  }
  if (!pack.parentCompare?.bullets?.length || pack.parentCompare.bullets.length < 3) {
    console.warn(
      `[va-tier2] ${pack.countySlug}: parentCompare needs ≥3 concrete bullets`
    );
  }
  if (pack.h1 && !/^Moving in /i.test(pack.h1)) {
    console.warn(
      `[va-tier2] ${pack.countySlug}: H1 should be role-forward “Moving in …” (got: ${pack.h1})`
    );
  }
  if (pack.h1 && /Movers Serving/i.test(pack.h1)) {
    console.warn(
      `[va-tier2] ${pack.countySlug}: H1 must not use “Movers Serving…”`
    );
  }
  ...
  const merged = [
    ...localResources,
    ...VA_TIER2_COMMON_RESOURCES.filter((r) => !seen.has(r.href)),
  ];
  ...
  return {
    ...
    resources: {
      ...
      intro:
        ...
        'Official links first; directory listings are independent, not endorsements. Verify Virginia DMV household-goods / motor-carrier authority for in-state moves and FMCSA for interstate legs before deposits.',
      items: merged,
    },
  };
}
