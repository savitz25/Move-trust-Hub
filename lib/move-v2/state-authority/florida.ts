...
export function parseFloridaBusinessSearchHtml(html: string, program: "IM" | "MB"): StateAuthorityRecord[] {
  ...
  const records = blocks.flatMap((block) => {
    ...
    const email = contactBlock.match(/<b>Email:<\/b>\s*([^<&\s]+)/i)?.[1]?.trim().toLowerCase();
    ...
    return [{
      state: "FL" as const, authorityType: program === "IM" ? "FL_IM" as const : "FL_MB" as const,
      ...
      email, sourceRecordReference: `FDACS:${program}:${license[1]}`,
    }];
  });
  return [...new Map(records.map((record) => [record.licenseNumber, record])).values()];
}
