...
export async function findLocalMoversByOriginCounty(
  ...
  return (
    await db.query(
      `${base} and o.state=$1 and o.county_geoid=$2 and o.zcta is null order by o.evidence_tier,o.distance_miles nulls last,f.display_name,o.provider_id`,
      [state, countyGeoid],
    )
  ).rows;
}
...
export async function findLocalMoversByOriginZip(
  ...
  const providers = (
    await db.query(
      `${base} and o.zcta=$1 order by o.evidence_tier,o.distance_miles nulls last,f.display_name,o.provider_id`,
      [resolution.zcta],
    )
  ).rows;
  return { resolution, providers };
...
export function mayEnterOriginSearch(input:{state:string;eligibility:string;role:"MOVER"|"BROKER";identityResolved:boolean;vertical?:"MOVE"|"AUTO_TRANSPORT";subscription?:string;rating?:number}) {
  return ["FL","WA"].includes(input.state) && input.eligibility === "STATE_VERIFIED_LOCAL_MOVER" && input.role === "MOVER" && input.identityResolved && input.vertical !== "AUTO_TRANSPORT";
}
