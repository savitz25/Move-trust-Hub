...
export function validTransferRecord(v: unknown): v is TransferRecord {
  ...
  return keys(r, ['browserHash','manifest','parentStage','continuationRef','requestPrefix','expiresAt','accountContextRef','projectRef']) &&
...
export class PostgresTransferStore implements TransferStore {
  ...
  /** Explicit bounded operator call, never invoked by the web handler or a cron. */
