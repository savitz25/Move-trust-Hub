...
/** JSON-LD for /moving-to/washington cluster parent */
export function buildWashingtonClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const washingtonHubs = getClusterMarkets('washington').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Washington', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#washington-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#washington-hub-list`,
        name: 'Washington City Moving Guides',
        numberOfItems: washingtonHubs.length,
        itemListElement: washingtonHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, WA`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/wisconsin cluster parent */
export function buildWisconsinClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const wisconsinHubs = getClusterMarkets('wisconsin').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Wisconsin', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#wisconsin-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#wisconsin-hub-list`,
        name: 'Wisconsin City Moving Guides',
        numberOfItems: wisconsinHubs.length,
        itemListElement: wisconsinHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, WI`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/colorado cluster parent */
export function buildColoradoClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const coloradoHubs = getClusterMarkets('colorado').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Colorado', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#colorado-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#colorado-hub-list`,
        name: 'Colorado City Moving Guides',
        numberOfItems: coloradoHubs.length,
        itemListElement: coloradoHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, CO`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/delaware cluster parent */
export function buildDelawareClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const delawareHubs = getClusterMarkets('delaware').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Delaware', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#delaware-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#delaware-hub-list`,
        name: 'Delaware City Moving Guides',
        numberOfItems: delawareHubs.length,
        itemListElement: delawareHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, DE`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/kansas cluster parent */
export function buildKansasClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const kansasHubs = getClusterMarkets('kansas').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Kansas', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#kansas-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#kansas-hub-list`,
        name: 'Kansas City Moving Guides',
        numberOfItems: kansasHubs.length,
        itemListElement: kansasHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, KS`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/kentucky cluster parent */
export function buildKentuckyClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const kentuckyHubs = getClusterMarkets('kentucky').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Kentucky', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#kentucky-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#kentucky-hub-list`,
        name: 'Kentucky City Moving Guides',
        numberOfItems: kentuckyHubs.length,
        itemListElement: kentuckyHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, KY`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/maryland cluster parent */
export function buildMarylandClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const marylandHubs = getClusterMarkets('maryland').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Maryland', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#maryland-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#maryland-hub-list`,
        name: 'Maryland City Moving Guides',
        numberOfItems: marylandHubs.length,
        itemListElement: marylandHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, MD`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/minnesota cluster parent */
export function buildMinnesotaClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const minnesotaHubs = getClusterMarkets('minnesota').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Minnesota', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#minnesota-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#minnesota-hub-list`,
        name: 'Minnesota City Moving Guides',
        numberOfItems: minnesotaHubs.length,
        itemListElement: minnesotaHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, MN`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/missouri cluster parent */
export function buildMissouriClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const missouriHubs = getClusterMarkets('missouri').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Missouri', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#missouri-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#missouri-hub-list`,
        name: 'Missouri City Moving Guides',
        numberOfItems: missouriHubs.length,
        itemListElement: missouriHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, MO`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/nevada cluster parent */
export function buildNevadaClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const nevadaHubs = getClusterMarkets('nevada').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Nevada', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#nevada-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#nevada-hub-list`,
        name: 'Nevada City Moving Guides',
        numberOfItems: nevadaHubs.length,
        itemListElement: nevadaHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, NV`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
...
/** JSON-LD for /moving-to/utah cluster parent */
export function buildUtahClusterSchemaGraph(
  title: string,
  description: string,
  canonicalPath: string
) {
  const canonical = `${SITE_URL}${canonicalPath}`;
  const published = new Set(getPublishedCityHubSlugs());
  const utahHubs = getClusterMarkets('utah').filter((market) =>
    published.has(market.slug)
  );
    ...
    '@graph': [
      organizationSchema,
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Utah', item: canonical },
        ],
      },
      {
        '@type': 'WebPage',
        ...
        mainEntity: { '@id': `${canonical}#utah-hub-list` },
      },
      {
        '@type': 'ItemList',
        '@id': `${canonical}#utah-hub-list`,
        name: 'Utah City Moving Guides',
        numberOfItems: utahHubs.length,
        itemListElement: utahHubs.map((market, index) => ({
          '@type': 'ListItem',
          position: index + 1,
          name: `${market.displayName}, UT`,
          url: `${SITE_URL}${getMarketPath(market)}`,
        })),
      },
    ],
