-- Builder 3 rebuild: PREPARED ONLY; not an automatic migration.
...
create table mth_profile_transfer.stages (
  ...
  check(record - array['browserHash','manifest','bindings','parentStage','continuationRef','requestPrefix','expiresAt','accountContextRef','projectRef'] = '{}'::jsonb),
...
create table mth_profile_transfer.nonces (
 key_hash text primary key check(key_hash ~ '^[a-f0-9]{64}$'),
 expires_at timestamptz not null check(expires_at>clock_timestamp() and expires_at<=clock_timestamp()+interval '34 seconds')
);
create index nonces_expiry on mth_profile_transfer.nonces(expires_at);
create table mth_profile_transfer.browsers (
 browser_hash text primary key check(browser_hash ~ '^[a-f0-9]{64}$'),
 expires_at timestamptz not null check(expires_at>clock_timestamp() and expires_at<=clock_timestamp()+interval '1 day')
);
create index browsers_expiry on mth_profile_transfer.browsers(expires_at);
alter table mth_profile_transfer.nonces enable row level security;
alter table mth_profile_transfer.nonces force row level security;
alter table mth_profile_transfer.browsers enable row level security;
alter table mth_profile_transfer.browsers force row level security;
revoke all on mth_profile_transfer.nonces,mth_profile_transfer.browsers from public,anon,authenticated;
grant select,insert,delete on mth_profile_transfer.nonces,mth_profile_transfer.browsers to mth_move_profile_transfer;
create policy source_service_only on mth_profile_transfer.nonces to mth_move_profile_transfer using(true) with check(true);
create policy source_service_only on mth_profile_transfer.browsers to mth_move_profile_transfer using(true) with check(true);
-- Capability only. No login mapping/password/provisioning without separately approved target packet.
commit;
