...
export async function generateStaticParams() {
  return getPublishedCityHubSlugs()
    .filter((slug) => getMarketBySlug(slug)?.clusterParent === 'new-york')
    .map((slug) => ({ slug }));
}
...
export default async function NewYorkCityHubPage({ params }: Props) {
  const { slug } = await params;
  const market = getMarketBySlug(slug);
  const content = getCityHubContent(slug);

  if (!market || !content || market.clusterParent !== 'new-york') notFound();
