...
import { SWEET_SPOT_COMMUTER_TOWNS_HUB_AREAS, SWEET_SPOT_COMMUTER_TOWNS_LENDER_SLUGS } from '@/lib/lender/mortgage/newJerseyLenders';
import { getLenderBySlug } from '@/lib/lender/lenders';

export const metadata: Metadata = {
  title: 'Cranford & Montclair Mortgage Lenders — Sweet Spot Commuter Towns (2026)',
  description:
    'NMLS-verified mortgage lenders in Cranford, Rahway, Edison, New Brunswick, Montclair, and Maplewood. 95%+ above-ask sales, 11–13 day contracts, Helix bio-tech growth, and direct rail to Manhattan.',
  alternates: { canonical: 'https://www.movetrusthub.com/lender/local-lenders/new-jersey/sweet-spot-commuter-towns' },
};
...
export default function SweetSpotCommuterTownsHubPage() {
  const hubLenders = SWEET_SPOT_COMMUTER_TOWNS_LENDER_SLUGS.map((slug) => getLenderBySlug(slug)).filter(Boolean);
      ...
      <div className="container mx-auto px-4 py-10">
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-8">
            <section>
              <h2 className="mb-4 text-2xl font-bold text-[#0A2540]">Commuter Sweet Spots — Start Here</h2>
              <div className="grid gap-3 sm:grid-cols-2">
                {SWEET_SPOT_COMMUTER_TOWNS_HUB_AREAS.map((c) => (
            ...
            <section className="rounded-2xl border border-zinc-200 bg-zinc-50 p-6">
              <h2 className="text-xl font-bold text-[#0A2540]">Why These Commuter Towns Are Different</h2>
              <ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-zinc-700">
                <li>More than 95% of listings sell above ask across these high-momentum commuter corridors</li>
                <li>Cranford homes sell in just 13 days with a $795K median on the Raritan Valley line</li>
                <li>Rahway offers a $525K entry point as the premier emerging value play</li>
                <li>Edison and New Brunswick surge past $605K driven by the Helix bio-tech district</li>
                <li>Montclair and Maplewood see 11-day pending times with 105%+ sale-to-list ratios</li>
              </ul>
            </section>
            <section>
              <h2 className="mb-4 text-2xl font-bold text-[#0A2540]">Top Verified Lenders — Sweet Spot Commuter Towns</h2>
              <div className="space-y-4">
                {hubLenders.map((lender, i) => (
                  lender && (
                    <LenderCard key={lender.id} lender={lender} rank={i + 1} countyLabel="Commuter Towns" />
                  )
                ))}
              </div>
            </section>
            ...
            <LeadCaptureForm stateName="Sweet Spot Commuter Towns" categoryId="mortgage" variant="state-page-v2" />
          </div>
          <aside className="space-y-6">
            ...
            <LeadCaptureForm stateName="Sweet Spot Commuter Towns" categoryId="mortgage" variant="sidebar-minimal" />
          </aside>
        </div>
      </div>
    </>
