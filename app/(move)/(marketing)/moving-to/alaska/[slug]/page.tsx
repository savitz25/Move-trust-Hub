...
export async function generateStaticParams() {
  return ssgParams(getPublishedCityHubSlugs()
    .filter((slug) => getMarketBySlug(slug)?.clusterParent === 'alaska')
    .map((slug) => ({ slug })));
}
...
export default async function AlaskaCityHubPage({ params }: Props) {
  const { slug } = await params;
  const market = getMarketBySlug(slug);
  const content = getCityHubContent(slug);

  if (!market || !content || market.clusterParent !== 'alaska') notFound();
