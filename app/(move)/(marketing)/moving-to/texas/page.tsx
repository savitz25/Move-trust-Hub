...
import {
  texasClusterContent,
  type TexasCorridorCity,
} from '@/lib/destinations/content/texas-tx';
...
import { buildTexasClusterSchemaGraph } from '@/lib/seo/build-destination-index-schema';
import { SITE_URL, buildOpenGraph, buildTwitter } from '@/lib/seo/site-metadata';

const content = texasClusterContent;
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-4">
      {cities.map((city) => {
        ...
        const path = market ? getMarketPath(market) : `/moving-to/texas/${city.slug}`;
...
export default function TexasClusterPage() {
  const published = new Set(getPublishedCityHubSlugs());
  const otherTexasCities = getClusterMarkets('texas').filter(
  ...
  return (
    <>
      <JsonLd
        data={buildTexasClusterSchemaGraph(
          content.seo.title,
      ...
      <div className="min-h-screen">
        <section className="relative border-b bg-gradient-to-b from-primary/5 to-background py-12 md:py-20">
          <div className="container mx-auto px-4 max-w-6xl">
            ...
            <DestinationClusterHeroCta clusterSlug="texas" clusterLabel="Texas" />
          </div>
        </section>
          ...
          <section className="py-12 md:py-16 bg-muted/20">
            <div className="container mx-auto px-4 max-w-6xl">
              <h2 className="text-2xl font-semibold tracking-tight mb-6">
                More Texas Cities — Coming Soon
              </h2>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {otherTexasCities.map((market) => (
