...
export async function generateStaticParams() {
  return getPublishedCityHubSlugs()
    .filter((slug) => getMarketBySlug(slug)?.clusterParent === 'delaware')
    .map((slug) => ({ slug }));
}
...
export default async function DelawareCityHubPage({ params }: Props) {
  const { slug } = await params;
  const market = getMarketBySlug(slug);
  const content = getCityHubContent(slug);

  if (!market || !content || market.clusterParent !== 'delaware') notFound();
