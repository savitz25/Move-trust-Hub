...
import {
  runProviderEnrichmentRefresh,
  type ProviderRefreshMode,
} from '@/lib/insurance/enrichment/run-refresh';
import { exportEnrichmentStoreFromDb } from '@/lib/insurance/enrichment/persist';

export const runtime = 'nodejs';
export const maxDuration = 300;

function parseMode(request: Request): ProviderRefreshMode {
...
async function handleRefresh(request: Request) {
  ...
  let body: { mode?: ProviderRefreshMode; limit?: number; offset?: number; slug?: string } = {};
  ...
  return NextResponse.json(
    {
      ...result,
      triggeredBy:
        source === 'admin' ? 'admin' : request.headers.get('x-github-action') ? 'github' : 'cron',
