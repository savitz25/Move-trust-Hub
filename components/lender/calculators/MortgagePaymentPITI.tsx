      ...
      <div className="grid gap-8 lg:grid-cols-2">
        <div className="space-y-4">
          ...
          <p className="text-sm text-[#6B7280]">
            Down payment: <strong className="text-[#111827]">{formatCurrency(computed.down$)}</strong>
          </p>
          <CalcSlider label="Interest Rate" value={Number(s.rate)} min={2} max={12} step={0.125}
            onChange={(v) => update({ rate: v })} suffix="%" />
          <div>
            <label htmlFor="term-select" className="text-sm font-medium text-[#111827]">
              Loan Term
            </label>
            <select
              id="term-select"
              ...
              className="mt-1 w-full rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-sm text-[#111827] shadow-sm focus:border-[#3B82F6] focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/20"
            >
              {[10, 15, 20, 30].map((t) => (
          ...
          <details className="rounded-xl border border-[#E5E7EB] bg-white">
            <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-[#111827]">
            ...
            <div className="space-y-3 border-t border-[#E5E7EB] bg-[#F9FAFB] p-4">
                ...
                onChange={(e) => {
                  ...
                  update({
                    ...
                    taxAmount: Math.round(
                      (Number(s.homePrice) * (STATE_TAX_RATES[st] ?? 1.1)) / 100,
                    ),
                  });
                }}
                className="w-full rounded-lg border border-[#E5E7EB] bg-white px-3 py-2 text-sm text-[#111827] focus:border-[#3B82F6] focus:outline-none focus:ring-2 focus:ring-[#3B82F6]/20"
              >
              ...
              {computed.ltv > 80 && (
                <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
                  LTV &gt; 80% — PMI estimated until 20% equity.
                </p>
              )}
            </div>
          </details>
          <details className="rounded-xl border border-[#E5E7EB] bg-white" open>
            <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-[#111827]">
        ...
        <div className="space-y-4">
          ...
          <CalcLineChart
            data={lineData}
            lines={[
              { key: 'balance', color: '#2563EB', label: 'With Extras' },
              { key: 'standard', color: '#9CA3AF', dashed: true, label: 'Standard' },
            ]}
          />
          <div className="flex flex-wrap gap-2">
            ...
            <select
              value={tableMode}
              onChange={(e) => setTableMode(e.target.value as 'monthly' | 'yearly')}
          ...
          {showTable && (
            <div className="max-h-64 overflow-auto rounded-lg border border-[#E5E7EB] bg-white">
              <table className="w-full text-xs text-[#374151]">
                <thead className="sticky top-0 bg-[#F3F4F6]">
                  <tr>
                    <th className="p-2 text-left font-semibold text-[#111827]">
                      {tableMode === 'yearly' ? 'Year' : 'Month'}
                    </th>
                    <th className="p-2 text-right font-semibold text-[#111827]">Principal</th>
                    <th className="p-2 text-right font-semibold text-[#111827]">Interest</th>
                    <th className="p-2 text-right font-semibold text-[#111827]">Balance</th>
                  </tr>
                </thead>
                <tbody>
                  {tableMode === 'yearly'
                    ? yearlySummary(computed.adj.schedule).map((r) => (
                        <tr key={r.year} className="border-t border-[#E5E7EB]">
                    ...
                    : computed.adj.schedule.slice(0, 120).map((r) => (
                        <tr key={r.month} className="border-t border-[#E5E7EB]">
