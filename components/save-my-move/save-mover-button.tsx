...
function ProfileSave({ companySlug, companyName, variant = 'icon', className, parentSaveEligible = false }: SaveMoverButtonProps) {
  ...
  return <span className="inline-flex min-w-0 flex-col items-start gap-1" data-ph-no-capture="true">
    ...
    <span id={statusId} role="status" aria-live="polite" aria-atomic="true"
      className={cn('max-w-64 text-xs break-words', failed ? 'text-red-800' : 'text-slate-700')}>
      {statusMessage}
    </span>
    {localSaved && parentSaveEligible && variant === 'button'
