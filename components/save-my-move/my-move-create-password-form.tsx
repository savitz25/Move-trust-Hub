...
import {
  setMyMovePasswordAction,
  skipMyMovePasswordPromptAction,
...
const BENEFITS = [
  {
    ...
    body: 'Sign in with email + password when you want faster access to Move HQ.',
  },
  {
    icon: Link2,
  ...
  },
  {
    ...
    body: 'Change or turn off password anytime under My Move → Settings.',
  },
] as const;

export function MyMoveCreatePasswordForm({ nextPath, email }: Props) {
  ...
  function onCreate(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await setMyMovePasswordAction({ password, confirm });
      if (!result.success) {
  ...
  function onSkip() {
    setError(null);
    startTransition(async () => {
      const result = await skipMyMovePasswordPromptAction();
      if (!result.success) {
      ...
      <div className="relative overflow-hidden rounded-2xl border bg-card shadow-lg shadow-black/5">
        ...
        <div className="space-y-8 p-6 sm:p-8">
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row sm:items-start">
              ...
              <div className="space-y-2 min-w-0">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed max-w-md mx-auto sm:mx-0">
                  You already have your personalized Move Plan. Add a password for convenience — or
                  keep using magic links. Nothing required.
                </p>
              </div>
            </div>
          ...
          <form onSubmit={onCreate} className="space-y-5">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="my-move-new-password" className="text-sm font-medium">
                  Create password
                </Label>
                <Input
                  id="my-move-new-password"
                  type="password"
              ...
              <div className="space-y-2">
                <Label htmlFor="my-move-confirm-password" className="text-sm font-medium">
                  Confirm password
                </Label>
                <div className="relative">
                  <Input
                    id="my-move-confirm-password"
                    type="password"
                    autoComplete="new-password"
                    value={confirm}
          ...
          <p className="text-center text-xs text-muted-foreground leading-relaxed px-2">
            ...
            <span className="font-medium text-foreground/80">My Move → Settings</span>.
          </p>
        </div>
      </div>
    </div>
