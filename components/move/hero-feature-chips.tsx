    ...
    >
      {chips.map((label) => (
        <li
          key={label}
          className={cn(
            'inline-flex max-w-full items-center gap-1.5 rounded-full border border-primary/15',
            'bg-primary/[0.06] px-2.5 py-1.5 text-[11px] font-medium leading-snug text-[#3d4f63]',
            'sm:px-3 sm:text-sm dark:border-primary/25 dark:bg-primary/10 dark:text-slate-200'
          )}
        >
          <span
            className="inline-flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground sm:h-4 sm:w-4"
            aria-hidden
          >
