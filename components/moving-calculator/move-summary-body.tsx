...
export type MoveSummaryBodyProps = {
  ...
  /** Larger touch targets for phone layouts */
  touchFriendly?: boolean;
};

export function MoveSummaryBody({
  inventory,
  groupedByRoom,
  totalVolume,
  totalItems,
  ...
  const qtyBtn = touchFriendly ? 'h-11 w-11' : 'h-6 w-6';
  const roomBtn = touchFriendly ? 'min-h-12 py-3' : 'py-2';
        ...
        <div className="rounded-lg border border-dashed border-primary/30 bg-primary/5 p-3">
          <div className="flex items-start gap-2">
            ...
            <Button
              ...
              className={cn('shrink-0 text-xs', touchFriendly && 'min-h-11 px-4')}
            >
              Add boxes
            </Button>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Button
              ...
              size={touchFriendly ? 'default' : 'sm'}
              onClick={onClear}
              className={cn('text-destructive hover:text-destructive text-xs', touchFriendly && 'min-h-11')}
            >
              Clear all
            </Button>
          </div>
            ...
            return (
              <div key={room} className="rounded-lg border overflow-hidden">
                <button
                  ...
                  className={cn(
                    'w-full flex items-center justify-between px-3 text-sm font-medium hover:bg-muted/30 transition-colors',
                    roomBtn
                  )}
                ...
                >
                  <span>
                    {room}{' '}
                    <span className="text-muted-foreground font-normal">({Math.round(roomVol)} cu ft)</span>
                  </span>
                  {expanded ? (
                      ...
                      <div className="px-2 pb-2 space-y-1">
                        {items.map((item) => (
                          <div
                            key={item.id}
                            className={cn(
                              'flex items-center gap-1.5 rounded-md bg-muted/20 px-2 text-xs',
                              touchFriendly ? 'py-2.5' : 'py-1.5'
                            )}
                          >
                            ...
                            <div className="flex items-center gap-0.5 shrink-0">
                              ...
                              >
                                <Minus className={touchFriendly ? 'h-4 w-4' : 'h-3 w-3'} />
                              </Button>
                              ...
                              >
                                <Plus className={touchFriendly ? 'h-4 w-4' : 'h-3 w-3'} />
                              </Button>
                              <Button
                                size="icon"
                                variant="ghost"
                                className={cn(qtyBtn, 'text-destructive hover:text-destructive')}
