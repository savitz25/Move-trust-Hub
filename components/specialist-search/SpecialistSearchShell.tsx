...
export function SpecialistSearchShell({ query = '', compact = false }: { query?: string; compact?: boolean }) {
  return (
    <section className={`rounded-2xl border border-[#E2E8F0] bg-white ${compact ? 'p-4 sm:p-5' : 'p-5 sm:p-7'}`} aria-labelledby={compact ? 'home-specialist-search' : 'specialist-search'}>
      ...
      <form action="/ask" method="get" role="search" aria-label="Research moving companies" className="mt-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <input id={compact ? 'home-move-research' : 'move-research'} name="q" defaultValue={query} maxLength={180} required
        ...
        <details className="mt-4 rounded-xl border border-[#E2E8F0] bg-[#F8FAFC] p-3">
          ...
          <div className="grid gap-3 pt-3 sm:grid-cols-3">
            <label className="text-sm font-semibold text-[#0A2540]">Role<select name="role" className="mt-1 min-h-11 w-full rounded-lg border bg-white px-2 font-normal"><option value="">Any role</option><option value="carrier">Carrier</option><option value="broker">Broker</option><option value="carrier_broker">Carrier/Broker</option></select></label>
            <label className="text-sm font-semibold text-[#0A2540]">Recorded state<select name="state" className="mt-1 min-h-11 w-full rounded-lg border bg-white px-2 font-normal"><option value="">Any state</option><option value="FL">Florida</option><option value="NJ">New Jersey</option><option value="CA">California</option></select></label>
            <label className="text-sm font-semibold text-[#0A2540]">Authority<select name="authority" className="mt-1 min-h-11 w-full rounded-lg border bg-white px-2 font-normal"><option value="">Any recorded state</option><option value="current">Current</option><option value="not_current">Not current</option></select></label>
          </div>
