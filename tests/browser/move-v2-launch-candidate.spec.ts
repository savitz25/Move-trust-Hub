...
test('same URL launch composition and hardened reads', async ({ page, request }) => {
  await page.goto('/'); await expect(page).toHaveTitle(/Move/); await expect(page.locator('form:visible').filter({hasText:'Evidence-first mover research'}).first()).toBeVisible({timeout:15000});
